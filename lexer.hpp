...
// determine token type based on final
// state------------------------------------------
...
public:
  ...
  void rule_print_helper() { // ofstream& outfile
    // token_print_helper();
    std::string tokenp;
    std::string lexemep;
    tokenp = token();
...
struct symbol_table_element {
  ...
  void print_element(ofstream &outfile) {
    std::cout << Identifier << "           " << MemoryLocation << "           "
              << Type << "\n";
    outfile << Identifier << "           " << MemoryLocation << "           "
            << Type << "\n";
  }
};
